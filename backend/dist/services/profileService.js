...
const PERIODS = new Set(['all', 'week', 'month']);
function pad(n) {
    return String(n).padStart(2, '0');
}
export function startOfWeekMondayKey(date = new Date()) {
...
function periodStartKey(period) {
    if (period === 'week')
        return startOfWeekMondayKey();
    if (period === 'month')
        return startOfMonthKey();
    return null;
}
function aggregate(list, exerciseLookup, groupLookup) {
    let totalSets = 0;
    let totalReps = 0;
    let totalVolume = 0;
    let failureSets = 0;
...
function favoriteExercise(stats) {
...
function currentStreak(allWorkouts) {
...
export async function getStats(userId, { period = 'all' } = {}) {
    ...
    weekWorkouts.forEach((w) => {
        ...
        w.entries.forEach((entry) => {
            ...
            if (!groupId || seenInDay.has(groupId))
                return;
...
export async function updateProfile(userId, patch = {}) {
    ...
    if (patch.profilePhotoUrl !== undefined) {
        ...
        if (url && !/^(https?:|data:image\/)/i.test(url)) {
            throw new HttpError(400, 'profilePhotoUrl must be an http(s) URL or a data:image URL');
        }
    ...
    return userRepo.publicView(updated);
}
