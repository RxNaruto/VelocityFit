...
import * as realtime from './realtimeService.js';
/**
...
function isNextDay(prevKey, key) {
    ...
    const nKey = `${next.getFullYear()}-${String(next.getMonth() + 1).padStart(2, '0')}-${String(next.getDate()).padStart(2, '0')}`;
    return nKey === key;
}
function streakBonus(streakDays) {
    return Math.min(streakDays * POINTS.STREAK_PER_DAY, POINTS.STREAK_BONUS_CAP);
}
export function pointsForWorkout(workout, streakDays = 1) {
    if (!workout || !Array.isArray(workout.entries))
        return 0;
 ...
 */
export async function totalPointsForUser(userId) {
 ...
 */
export async function onWorkoutChanged(userId) {
    ...
    // Broadcast in the background — never let realtime delivery slow down the
    // user-facing write.
    Promise.all([leaderboardService.getSnapshot(), leaderboardService.getRank(userId)])
        .then(([snapshot, rank]) => {
        realtime.publish(realtime.EVENTS.LEADERBOARD_UPDATED, snapshot);
        realtime.publish(realtime.EVENTS.USER_UPDATED, {
            userId,
            points: total,
            rank: rank.rank,
        });
    })
        .catch((err) => console.error('[points] realtime publish failed:', err));
    return total;
}
